...
Detects `deinit` functions with pointer receivers that don't set the receiver to `undefined`. The diagnostic uses the receiver's actual parameter name, such as `self.*` or `value.*`. This is a Zig convention to help catch use-after-free bugs during development.
